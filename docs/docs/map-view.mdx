...
<BrowserOnly>
  {() => {
    ...
    const MapViewCore = require('../../lib/map').MapViewCore
    const MapContextProvider = require('../../lib/map').MapContextProvider

    const OtherUI = () => {
      const MapContext = require('../../lib/map').MapContext
...
For more information on MapView events, see [the ArcGIS JS API documentation](https://developers.arcgis.com/javascript/latest/api-reference/esri-views-MapView.html#on).  See ESRIACT examples here TODO