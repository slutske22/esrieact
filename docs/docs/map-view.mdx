...
Other ESRIEACT map components, like [layers](category/layer-components), or [widgets](category/widgets).
...
The `ViewProperties` prop can take an `events` field, which is an `ViewEventHandlerFnMap`, or a function that takes the `MapView` instance as a parameter, and returns a `ViewEventHandlerFnMap`.  A `ViewEventHandlerFnMap` is an object whose keys are possible events on a `MapView`, and whose values are the event handlers to be called on those events:
