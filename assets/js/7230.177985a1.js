...
function getEffectiveReferenceElement(tooltip) {
  ...
  return (typeof referenceElement === "string" ? (0,dom.q)(tooltip, { id: referenceElement }) : referenceElement) || null;
}
class TooltipManager {
  ...
  getReferenceElShadowRootNode(referenceEl) {
    return referenceEl instanceof Element ? (0,dom.c)(referenceEl) : null;
  }
}
const styles = (0,css_tag/* css */.AH)`:host{display:contents;--calcite-floating-ui-z-index: var(--calcite-tooltip-z-index, var(--calcite-z-index-tooltip))}.position-container{inline-size:max-content;display:none;max-inline-size:100vw;max-block-size:100vh;inset-block-start:0;left:0;z-index:var(--calcite-floating-ui-z-index)}.position-container .calcite-floating-ui-anim{position:relative;transition:var(--calcite-floating-ui-transition);transition-property:inset,left,opacity;opacity:0;box-shadow:0 0 16px #00000029;z-index:var(--calcite-z-index);border-radius:.25rem}.position-container[data-placement^=bottom] .calcite-floating-ui-anim{inset-block-start:-5px}.position-container[data-placement^=top] .calcite-floating-ui-anim{inset-block-start:5px}.position-container[data-placement^=left] .calcite-floating-ui-anim{left:5px}.position-container[data-placement^=right] .calcite-floating-ui-anim{left:-5px}.position-container[data-placement] .calcite-floating-ui-anim--active{opacity:1;inset-block-start:0;left:0}.calcite-floating-ui-arrow{pointer-events:none;position:absolute;z-index:calc(var(--calcite-z-index) * -1);fill:var(--calcite-color-foreground-1)}.calcite-floating-ui-arrow__stroke{stroke:var(--calcite-color-border-3)}.container{position:relative;overflow:hidden;padding:.75rem 1rem;font-size:var(--calcite-font-size--2);line-height:1.375;font-weight:var(--calcite-font-weight-medium);border-radius:var(--calcite-tooltip-corner-radius, var(--calcite-corner-radius-round));color:var(--calcite-tooltip-text-color, var(--calcite-color-text-1));max-inline-size:20rem;max-block-size:20rem;text-align:start}.position-container .calcite-floating-ui-anim{border-width:1px;border-style:solid;background-color:var(--calcite-tooltip-background-color, var(--calcite-color-foreground-1));border-color:var(--calcite-tooltip-border-color, var(--calcite-color-border-3));border-radius:var(--calcite-tooltip-corner-radius, var(--calcite-corner-radius-round))}.calcite-floating-ui-arrow{fill:var(--calcite-tooltip-background-color, var(--calcite-color-foreground-1))}.calcite-floating-ui-arrow__stroke{stroke:var(--calcite-tooltip-border-color, var(--calcite-color-border-3))}:host([hidden]){display:none}[hidden]{display:none}`;
const manager = new TooltipManager();
class Tooltip extends dist/* LitElement */.WF {
  constructor() {
    super(...arguments);
    this.guid = `calcite-tooltip-${(0,guid.g)()}`;
    ...
    this.placement = "auto";
    this.calciteTooltipBeforeClose = (0,dist/* createEvent */.lh)({ cancelable: false });
    this.calciteTooltipBeforeOpen = (0,dist/* createEvent */.lh)({ cancelable: false });
    this.calciteTooltipClose = (0,dist/* createEvent */.lh)({ cancelable: false });
    this.calciteTooltipOpen = (0,dist/* createEvent */.lh)({ cancelable: false });
  }
  static {
  ...
  willUpdate(changes) {
    if (changes.has("offsetDistance") && (this.hasUpdated || this.offsetDistance !== floating_ui.d) || changes.has("offsetSkidding") && (this.hasUpdated || this.offsetSkidding !== 0) || changes.has("overlayPositioning") && (this.hasUpdated || this.overlayPositioning !== "absolute") || changes.has("placement") && (this.hasUpdated || this.placement !== "auto")) {
  ...
  render() {
    ...
    })} ${(0,ref/* ref */.K)(this.setTransitionEl)}>${(0,FloatingArrow.F)({ floatingLayout, ref: (arrowEl) => this.arrowEl = arrowEl })}<div class=${(0,dist/* safeClassMap */.CP)(CSS.container)}><slot></slot></div></div></div>`;
  }
}
(0,runtime.c)("calcite-tooltip", Tooltip);
